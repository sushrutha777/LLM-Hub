...
Start-Process "powershell" -ArgumentList "-NoExit", "-Command", "Title 'LLMHub API Gateway'; cd backend; `$env:PYTHONPATH='.'; venv\Scripts\python -m uvicorn gateway.app.main:app --host 0.0.0.0 --port 8000"

Write-Host "`n[3/4] Starting Model Router on Port 8001..." -ForegroundColor Yellow
Start-Process "powershell" -ArgumentList "-NoExit", "-Command", "Title 'LLMHub Model Router'; cd backend; `$env:PYTHONPATH='.'; venv\Scripts\python -m uvicorn model_router.app.main:app --host 0.0.0.0 --port 8001"

Write-Host "`n[4/4] Starting React Dashboard on Port 5173..." -ForegroundColor Yellow
Start-Process "powershell" -ArgumentList "-NoExit", "-Command", "Title 'LLMHub Frontend'; cd frontend; npm run dev"
